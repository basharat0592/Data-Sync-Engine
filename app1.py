...
# -------------------------
def stream_source_to_target(db_name, source_table, source_cols, target_table, target_columns, pk_cols, table_meta, chunk_size=50000, where_clause=None, where_params=None, cast_uuid=False):
    ...
    # Add db_name column if required
    include_db_name = table_meta.get("include_db_name", False)
    if include_db_name and "db_name" not in target_columns:
        target_columns.append("db_name")

    transform_map = table_meta.get("transform", {})
    ...
    while True:
        ...
        rows_tuples = []
        for r in rows:
            row_dict = dict(zip(source_cols, r))
            processed = []

            for src_col, tgt_col in zip(source_cols, target_columns):
                if tgt_col in transform_map:
                    value = transform_map[tgt_col](row_dict)
                else:
                    value = row_dict.get(src_col)
                processed.append(value)

            if include_db_name:
                processed.append(db_name)

            rows_tuples.append(tuple(processed))
...
def run_job(job):
    ...
    for table in job["tables"]:
        ...
        inc_col = table.get("incremental_column")
        ...
        if full_load:
            ...
            rows_copied = stream_source_to_target(db_name, source, source_cols, target, target_columns, pk, table, chunk_size=chunk_size, cast_uuid=cast_uuid)
            ...
            rows_copied = stream_source_to_target(db_name, source, source_cols, target, target_columns, pk, table, chunk_size=chunk_size, where_clause=where_clause, where_params=where_params, cast_uuid=cast_uuid)
            print(f"  Finished copying {rows_copied} rows into {target}")
            if rows_copied > 0 and inc_col:
                new_last_sync = query_source_max(source, inc_col)
                if new_last_sync:
                    save_last_sync(db_name, source, new_last_sync)
                    print(f"  Updated last_sync to {new_last_sync}")
...
def lookup_agency_id(row):
    pg_cur.execute("SELECT momentum_agency_id FROM domain.agencies WHERE momentum_agency_id = %s", (row["InsuranceAgencyId"],))
